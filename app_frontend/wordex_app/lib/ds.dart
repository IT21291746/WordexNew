// ignore_for_file: library_private_types_in_public_api, unused_field, unused_element, avoid_print, prefer_final_fields
...
class Ds extends StatefulWidget {
  ...
  const Ds({super.key, required this.userDetails, required this.summaryDetails,required this.racSummaryDetails,required this.ljSummaryDetails,required this.wjSummaryDetails});
...
class _DsState extends State<Ds> {
  ...
  Future<void> _fetchQuestions() async {
    try {
      final response = await http.get(Uri.parse('http://172.20.10.2:8080/dsq/random'));
  ...
  Widget _buildLoadingScreen() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 20),
        Text("Round 05", style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold)),
        Text("Spelling Test Round", style: GoogleFonts.poppins(fontSize: 18)),
        SizedBox(height: 20),
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            Map<String, int> dsSummaryDetails = {};
            for (int i = 0; i < _results.length; i++) {
              dsSummaryDetails["Q${i + 1}"] = _results[i]["correct"] == "Correct" ? 1 : 0;
            }

            // Add total correct answers and time spent
            dsSummaryDetails["totalCorrect"] = _totalCorrect;
            dsSummaryDetails["totalTimeSpent"] = _totalTimeSpent;
...
Navigator.pushReplacement(
  ...
  MaterialPageRoute(
    builder: (context) => Summary(
      ...
      wjSummaryDetails: widget.wjSummaryDetails,
      dsSummaryDetails: dsSummaryDetails,
